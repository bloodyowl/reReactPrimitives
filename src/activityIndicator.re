...
let make ::size ::color _children => {
  let setCanvasRef canvasRef state _self =>
    ReasonReact.SilentUpdate (
      switch (Js.Null.to_opt canvasRef) {
      | Some canvas => {
          ...state,
          context: Some (ReasonJs.CanvasElement.getContext2d canvas)
        }
      | None => state
      }
    );
  let draw state =>
    ...
    | Some context =>
      ...
      ReasonJs.Canvas2d.clearRect
        x::0.0 y::0.0 w::actualSize h::actualSize context;
      ReasonJs.Canvas2d.translate
        x::(actualSize /. 2.0) y::(actualSize /. 2.0) context;
      ReasonJs.Canvas2d.rotate 0.172665 context;
      ReasonJs.Canvas2d.translate
        x::(-. (actualSize /. 2.0)) y::(-. (actualSize /. 2.0)) context;
      let centeredArc =
        ReasonJs.Canvas2d.arc x::(actualSize /. 2.0) y::(actualSize /. 2.0);
      ...
      <canvas
        ...
        style=(
          ReactDOMRe.Style.make
            width::(sizeAttr ^ "px") height::(sizeAttr ^ "px") ()
        )
      />
    }
  }
